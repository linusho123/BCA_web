...
| — (no scenario asserts it) | `src/domain/plate.ts:193` | `NON_NUMERIC_CELL` at INFO fires for every well holding no measurement. Ruling 3 restricts the report to wells inside an assignment and raises it to warn. Verified by grep: no scenario in `features/` asserts this issue, so nothing green flips. | 3 |
